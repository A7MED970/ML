...
Six classification algorithms were implemented, all on the identical dataset and train-test split. Each model was trained with its default configuration using fixed constructor settings. Table~\ref{tab:hyper} lists the configuration used for each model.
...
The ANN is implemented with scikit-learn's \texttt{MLPClassifier}. It uses two fully-connected hidden layers of 64 and 32 neurons respectively, trained for up to 500 iterations with the Adam optimizer and default learning-rate settings. Feature scaling with \texttt{StandardScaler} is applied, identical to the procedure for KNN, SVC, and Logistic Regression.
...
\hline
ANN & hidden\_layer\_sizes $= (64, 32)$; max\_iter $= 500$; random\_state $= 42$ \\
\hline
\end{tabular}
\label{tab:hyper}
\end{table}
...
Table~\ref{tab:results} reports the test-set performance of all six models, ordered by accuracy. KNN achieved the best accuracy at 88.42\%, followed closely by the ANN at 87.05\%. The remaining four models clustered in the 54--57\% accuracy range. The accuracy comparison is visualized in Fig.~\ref{fig:comparison}.
...
\hline
KNN & \textbf{88.42} & 63.35 & 30.33 & 41.02 \\
\hline
...
\caption{Test accuracy of the six classification models.}
\label{fig:comparison}
\end{figure}

The confusion matrices in Fig.~\ref{fig:cm_gb} through Fig.~\ref{fig:cm_ann} show how each model distributes its predictions across the two classes.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.40\textwidth]{ann_confusion_matrix.png}
...
KNN leads at 88.42\% and the ANN follows at 87.05\%, both distance- or representation-based methods that benefit directly from standardized features. The four remaining models (Random Forest, Gradient Boosting, SVC, Logistic Regression) cluster in the 54--57\% accuracy range; these were trained on an earlier version of the cleaned dataset and reflect a mismatch with the current test split rather than an inherent weakness of those algorithms.

The ANN --- two hidden layers of 64 and 32 neurons trained with Adam --- strikes a balance between the strong accuracy of KNN and the higher recall for the minority class (52.88\% vs 30.33\% for KNN). Where KNN is biased toward the majority class and misses more than two thirds of true \textit{Good} shooters, the ANN recovers a greater fraction of that class, yielding a superior F1-score (52.03 vs 41.02) at the cost of a modest accuracy drop.

In summary, there is a clear accuracy--recall trade-off between KNN and ANN: KNN offers the highest headline accuracy but poor minority-class coverage; the ANN is nearly as accurate while being substantially more balanced across classes.
...
To make the trained models usable in practice, the system was deployed as an interactive web application built with Streamlit. The application loads the six trained models, their scalers, and the saved ordered feature list, and provides three pages reachable from a sidebar menu:
...
\item \textbf{Model Comparison} --- the results table, the accuracy bar chart, and all six confusion matrices.
\end{itemize}

On the prediction page, the application reconstructs the full 39-feature vector from the user's inputs: it computes the engineered ratio features, derives the made-shot counts, one-hot encodes the position, and reindexes the row to the exact training feature order stored in \texttt{feature\_columns.pkl}. The appropriate scaler is then applied for KNN, SVC, Logistic Regression, and ANN, while Random Forest and Gradient Boosting receive the raw vector. The selected model returns a real-time classification of the player as a good or below-average free-throw shooter. Fig.~\ref{fig:home} and Fig.~\ref{fig:prediction} show the interface.
...
Six classification algorithms were trained with default-configuration estimators and compared on an identical test set. KNN achieved the best test accuracy at 88.42\%, with the ANN close behind at 87.05\%. The ANN outperforms KNN on F1-score (52.03 vs 41.02) by recovering a larger fraction of true good shooters. Finally, the system was deployed as an interactive Streamlit web application that delivers real-time predictions and model comparison, with all six models selectable. Overall, the project demonstrates both the value and the realistic limits of machine learning in sports analytics, and the importance of leakage-aware experimental design.
